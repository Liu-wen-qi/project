...
router.post('/change',(req,res) => {
    ...
    for (let i in studnet){
        if (req.body.name ==studnet[i].name || oldName == studnet[i].name ){
            ...
            obj.math = req.body.pass;
            obj.Chinese = req.body.cpass;
            obj.English = req.body.sname;
            obj.English = req.body.phone;
            obj.English = req.body.email;
            req.body.total=Number(req.body.name) + Number(req.body.pass) + Number(req.body.cpass) + Number(req.body.sname) + Number(req.body.phone) + Number(req.body.email);
...
router.post('/search',(req,res) => {
    ...
    pattern(name);
    if (search.length == 0){
    res.send('<p \"style=color:red;\">没有'+name+"</P>");
    }else{
